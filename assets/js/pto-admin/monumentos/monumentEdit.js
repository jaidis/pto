...
$(document).ready(function() {
    ...
    $("#formDeleteComentario").submit(function(e) {
        ...
        $.ajax({
            method: "POST",
            url: "/pto-admin/monumentos/delete_comment",
            data: $("#formDeleteComentario").serialize(),
            success: function(data) {
                var x = jQuery.parseJSON(data);
                // console.log(x);
                $("#comentario_section").load("/pto-admin/monumentos/edit/"+configuracion[0].id+" #comentario_section > *", function() {
                    $('#closeModal').click();
                    lanzarToast(x.response, x.message);
                    generateTable();
                });
